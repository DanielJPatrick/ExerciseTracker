...
public class ExerciseSelectorAdapter extends BaseAdapter {
    private static final int EXERCISE_VH_TYPE = 1;

    public ExerciseSelectorAdapter(DExercise[] rawItems) {
        super(rawItems);
    }
    ...
    protected Object[] generateListItems(Object[] rawItems) {
        ...
        if(rawItems == null) {
            ...
            for(Object rawItemLooper : rawItems) {
                items.add(new ExerciseSelectorAdapter.ExerciseViewBinder((DExercise)rawItemLooper));
            }
        }
    ...
    public int getItemViewType(int position) {
        if(super.getItem(position) instanceof ExerciseSelectorAdapter.ExerciseViewBinder) {
            return ExerciseSelectorAdapter.EXERCISE_VH_TYPE;
        } else {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        switch(viewType) {
            case(ExerciseSelectorAdapter.EXERCISE_VH_TYPE):
                return new ExerciseSelectorAdapter.ExerciseViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_workout_exercise_list_exercise, parent, false));
        }
        return null;
    }
    ...
    public class ExerciseViewBinder extends BaseViewBinder {
        ...
        public void bind(BaseViewHolder viewHolder) {
            ((ExerciseSelectorAdapter.ExerciseViewHolder)viewHolder).nameTv.setText(this.exercise.getName());
        }
    }
}
